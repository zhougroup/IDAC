...
class TRPO(ActorCriticRLModel):
	...
	def __init__(self, policy, env, gamma=0.99, timesteps_per_batch=1024, max_kl=0.01, cg_iters=10, lam=0.98, ent_coef=0,
		...
		with SetVerbosity(self.verbose):
			...
			with self.graph.as_default():
				...
				with tf.variable_scope("loss", reuse=False):
					...
					entbonus = self.ent_coef_ph * meanent
					...
					#tmp = self.policy_pi.proba_distribution.neglogp(action)
					#surrgain = tf.reduce_mean(-tmp * atarg)
					...
					self.compute_losses = tf_util.function([observation, old_policy.obs_ph, action, atarg, self.ent_coef_ph], losses)
					self.compute_fvp = tf_util.function([flat_tangent, observation, old_policy.obs_ph, action, atarg],
														fvp)
					self.compute_vflossandgrad = tf_util.function([observation, old_policy.obs_ph, ret, self.ent_coef_ph],
				...
				self.compute_lossandgrad = \
					tf_util.function([observation, old_policy.obs_ph, action, atarg, ret, self.ent_coef_ph],
	...
	def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="TRPO",
			  reset_num_timesteps=True, path = None):

		new_tb_log = self._init_num_timesteps(reset_num_timesteps)
		self.ent_rate = get_schedule_fn(self.ent_coef)
		...
		with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
			...
			with self.sess.as_default():
				...
				iters_so_far = 0
				t_start = time.time()
				len_buffer = deque(maxlen=100)  # rolling buffer for episode lengths
				reward_buffer = deque(maxlen=100)  # rolling buffer for episode rewards
				...
				while True:
					...
					frac = 1.0 - (timesteps_so_far - 1.0) / total_timesteps
					ent_now = self.ent_rate(frac)                    
					...
					for k in range(self.g_step):
						...
						with self.timed("computegrad"):
							...
							if writer is not None:
								...
								_, grad, *lossbefore = self.compute_lossandgrad(*args, tdlamret, ent_now, sess=self.sess,
																				options=run_options,
																				run_metadata=run_metadata)
						...
						if np.allclose(grad, 0):
							...
							for _ in range(10):
								...
								mean_losses = surr, kl_loss, *_ = self.allmean(
									np.array(self.compute_losses(*args, ent_now, sess=self.sess)))
					...
					if len(len_buffer) > 0:
						with open(path,'a') as f1:
							f1.write("%f " % np.mean(reward_buffer))
					...
					with open(path,'a') as f1:
						f1.write("%f\n" % (self.num_timesteps/self.timesteps_per_batch))
