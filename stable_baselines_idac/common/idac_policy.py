...
from stable_baselines.common.idac_distributions import make_proba_dist_type, CategoricalProbabilityDistribution, \
...
def mlp_extractor(flat_observations, flat_noises, net_arch, act_fun, layer_norm=False):
    """
    ...
    latent_noise = tf.cast(latent_noise, tf.float32)
    latent_policy = tf.concat([latent,latent_noise],axis=1)
    latent_value = latent
...
def mlp_extractor_infer(flat_observations, net_arch, act_fun, layer_norm=False):
    """
    Constructs an MLP that receives observations as an input and outputs a latent representation for the policy and
    a value network. The ``net_arch`` parameter allows to specify the amount and size of the hidden layers and how many
    of them are shared between the policy network and the value network. It is assumed to be a list with the following
    ...
    """
    latent = flat_observations
    infer_only_layers = []  # Layer sizes of the network that only belongs to the policy network
    ...
    for idx, layer in enumerate(net_arch):
        if isinstance(layer, int):  # Check that this is a shared layer
            ...
            assert isinstance(layer, dict), "Error: the net_arch list can only contain ints and dicts"
            if 'infer' in layer:
                assert isinstance(layer['infer'], list), "Error: net_arch[-1]['pi'] must contain a list of integers."
                infer_only_layers = layer['infer']

            break  # From here on the network splits up in policy and value network

    # Build the non-shared part of the network
    latent_infer = latent
    for idx, infer_layer_size in enumerate(infer_only_layers):
        if infer_layer_size is not None:
            assert isinstance(infer_layer_size, int), "Error: net_arch[-1]['pi'] must only contain integers."
            if layer_norm:
                with tf.variable_scope("infer_fc{}".format(idx)):
                    n_input = latent_infer.get_shape()[1].value
                    weight = tf.get_variable("w", [n_input, infer_layer_size], initializer=ortho_init(np.sqrt(2)))
                    bias = tf.get_variable("b", [infer_layer_size], initializer=tf.constant_initializer(np.sqrt(2)))
                    pi_h = tf.matmul(latent_infer, weight) + bias
                    pi_h = tf.contrib.layers.layer_norm(pi_h, center=True, scale=True)
                latent_infer = act_fun(pi_h)
...
class BasePolicy(ABC):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, scale=False,
                 obs_phs=None, noise_phs=None, add_action_ph=False):
        self.n_env = n_env
        ...
        with tf.variable_scope("input", reuse=False):
            ...
            if noise_phs is None:

                self._noise_ph = tf.placeholder(dtype=ob_space.dtype, shape=(n_batch, noise_dim), name = "noise_ph")


            self._action_ph = None # comment for v7.
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, scale=False):
        super(ActorCriticPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=reuse,
                                                scale=scale)
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            self._action, self._gen_stdn = self.proba_distribution.sample()
            self._deterministic_action = self.proba_distribution.mode()
            self._neglogp = self.proba_distribution.neglogp(self.action)
            #self._neglogp_margin = self.proba_distribution.neglogp(self.action_ph) # added Mar11
    ...
    #@property
    #def neglogp_margin(self):
    #    return self._neglogp_margin
    ...
    @property
    def proba_distribution_std(self):
        return self._proba_distribution_std
...
class InferenceNet(ActorCriticPolicy):

    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, prior_std=1.0, reuse=False, layers=None, net_arch=None, layer_norm=False,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="mlp", **kwargs):
        super(InferenceNet, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))
        ...
        if net_arch is None:
            if layers is None:
                layers = [64, 64]
            net_arch = [dict(infer=layers)]        
        with tf.variable_scope("model", reuse=reuse):

            infer_latent = mlp_extractor_infer(tf.layers.flatten(self.processed_obs), net_arch, act_fun, self.layer_norm)

            self._proba_distribution, self._policy = \
                self.pdtype.proba_distribution_from_latent_infer(infer_latent, init_scale=0.01)
        
            self._proba_distribution_std, _ = self.pdtype.proba_distribution_from_latent_infer(infer_latent, init_scale=0.01, std_normal=True, prior_std=self.prior_std)

        self._value_fn = linear(infer_latent, 'not_useful', 1)
    ...
    def step(self, obs, state=None, mask=None):
        epsilon, ep_gen = self.sess.run([self.action, self.gen_stdn], {self.obs_ph: obs})
        return epsilon, ep_gen

    def step_deterministic(self, obs, state=None, mask=None):
        return self.sess.run(self.deterministic_action, {self.obs_ph: obs})
...
class MlpInference(InferenceNet):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, **_kwargs):
        super(MlpInference, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse,
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, layers=None, net_arch=None, layer_norm=False,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="mlp", **kwargs):
        super(FeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))
        self.layer_norm = layer_norm
        self._kwargs_check(feature_extraction, kwargs)
        self._proba_distribution_std = None
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                ...
                pi_latent, vf_latent = mlp_extractor(tf.layers.flatten(self.processed_obs), tf.layers.flatten(self.noise_ph), net_arch, act_fun, self.layer_norm)
    ...
    def step(self, obs, noise, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp],
                                                   {self.obs_ph: obs, self.noise_ph: noise})
        else:
            action, value, neglogp, stdn_gen = self.sess.run([self.action, self.value_flat, self.neglogp, self.stdn_gen],
                                                   {self.obs_ph: obs, self.noise_ph: noise})
        return action, value, self.initial_state, neglogp, stdn_gen

    def proba_step(self, obs, noise, state=None, mask=None):
        return self.sess.run(self.policy_proba, {self.obs_ph: obs, self.noise_ph:noise})

    def value(self, obs, state=None, mask=None):
        return self.sess.run(self.value_flat, {self.obs_ph: obs})

    #def margin_lik(self, obs, action, epsilon, state=None, mask=None):
    #    return self.sess.run(self.neglogp_margin, {self.obs_ph: obs, self.noise_ph: epsilon, self.action_ph: action})
...
class MlpPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, **_kwargs):
        super(MlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse,
...
class LnMlpPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=False, **_kwargs):
        super(LnMlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse,
                                        feature_extraction="mlp", layer_norm=True, **_kwargs)
