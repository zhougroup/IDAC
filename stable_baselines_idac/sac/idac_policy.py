...
from stable_baselines.common.idac_policy import BasePolicy, nature_cnn, register_policy
...
def gaussian_likelihood_np(input_, mu_, log_std):
	"""
	Helper to computer log likelihood of a gaussian.
	Here we assume this is a Diagonal Gaussian.
...
class SACPolicy(BasePolicy):
	...
	def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, noise_dim=None, reuse=False, scale=False):
		super(SACPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim, reuse=reuse, scale=scale)
	...
	def make_actor(self, obs=None, noise=None, reuse=False, scope="pi"):
		"""
	...
	def make_noise(self, obs=None, reuse=False, scope="inf"):
		"""
...
class FeedForwardPolicy(SACPolicy):
	...
	def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, noise_dim=None, reuse=False, layers=None,
				 cnn_extractor=nature_cnn, feature_extraction="cnn", reg_weight=0.0,
				 layer_norm=False, act_fun=tf.nn.relu, **kwargs):
		super(FeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, noise_dim,
	...
	def make_noise(self, obs=None, reuse=False, scope="inf"):
		...
		with tf.variable_scope(scope, reuse=reuse):
			inf_h = tf.layers.flatten(obs)
			epsilon = tf.random_normal([tf.shape(obs)[0],self.noise_dim])
			inf_h = tf.concat([inf_h, epsilon],axis=-1)
			inf_h = mlp(inf_h, self.layers, self.activ_fn, layer_norm=self.layer_norm)

			self.noise_mu = mu_ = tf.layers.dense(inf_h, self.noise_dim, activation=None)
			log_std = tf.layers.dense(inf_h, self.noise_dim, activation=None)
		log_std = tf.clip_by_value(log_std, LOG_STD_MIN, LOG_STD_MAX)
		
		self.noise_std = std = tf.exp(log_std)

		#noise = mu_ + tf.random_normal(tf.shape(mu_)) * std
		noise = self.noise_mu

		self.logp_pi_noise = gaussian_likelihood(noise, mu_, log_std)
		#self.noise_entropy = gaussian_entropy(log_std)
		# MISSING: reg params for log and mu
		# Apply squashing and account for it in the probabilty
		deterministic_noise, noise, self.logp_pi_noise = apply_squashing_func(mu_, noise, self.logp_pi_noise)
		## noise doesn't need squashing
		self.noise = noise
	...
	def make_actor(self, obs=None, noise=None, reuse=False, scope="pi"):
		...
		with tf.variable_scope(scope, reuse=reuse):
			if self.feature_extraction == "cnn":
				...
				epsilon = tf.random_normal([tf.shape(obs)[0],self.noise_dim])
				input_ = tf.concat([pi_h,noise,epsilon],axis=-1)

			pi_h = mlp(input_, self.layers, self.activ_fn, layer_norm=self.layer_norm)
		...
		# MISSING: reg params for log and mu
		# Apply squashing and account for it in the probabilty
		deterministic_policy, policy, logp_pi_new = apply_squashing_func(mu_, pi_, logp_pi)
		self.policy = deterministic_policy
		self.deterministic_policy = deterministic_policy

		return deterministic_policy, deterministic_policy, logp_pi_new,  pi_, mu_, log_std # last four elements not useful

	def make_critics(self, obs=None, action=None, noise=None, reuse=False, scope="values_fn",
		...
		with tf.variable_scope(scope, reuse=reuse):
			if self.feature_extraction == "cnn":
				...
				noise = tf.layers.flatten(noise)
				noise = tf.cast(noise, tf.float32)
				input_ = tf.concat([critics_h,noise],axis=-1)

			if create_vf:
				# Value function: only related to state (V(s))
				with tf.variable_scope('vf', reuse=reuse):
					vf_h = mlp(input_, self.layers, self.activ_fn, layer_norm=self.layer_norm) # changed from critics_h to vf_h, v6
					value_fn = tf.layers.dense(vf_h, 1, name="vf")
				self.value_fn = value_fn

			if create_qf:
				# Concatenate preprocessed state and action
				qf_h = tf.concat([input_, action], axis=-1)
	...
	def step(self, obs, noise, state=None, mask=None, deterministic=False):
		if deterministic:
			return self.sess.run(self.deterministic_policy, {self.obs_ph: obs, self.noise_ph: noise})
		return self.sess.run(self.policy, {self.obs_ph: obs, self.noise_ph: noise})

	def gen_noise(self, obs, state=None, mask=None, deterministic=False):
		if deterministic:
			return self.sess.run(self.deterministic_noise, {self.obs_ph: obs})
		return self.sess.run(self.noise, {self.obs_ph: obs})


	def proba_step(self, obs, noise, state=None, mask=None):
		return None

	def proba_noise(self, obs, state=None, mask=None):
		return None
...
class MlpPolicy(FeedForwardPolicy):
	...
	def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, noise_dim=100, reuse=False, **_kwargs):
...
class LnMlpPolicy(FeedForwardPolicy):
	...
	def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, noise_dim=None, reuse=False, **_kwargs):
