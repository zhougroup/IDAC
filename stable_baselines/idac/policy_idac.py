...
def gaussian_likelihood(input_, mu_, log_std):
	...
	Here we assume this is a Diaonal Gaussian.
...
class FeedForwardPolicy(SACPolicy):
	...
	def make_noise(self, obs=None, reuse=False, scope="inf"):
		...
		noise = tf.random_normal((tf.shape(self.processed_obs)[0], self.noise_dim))
	...
	def make_actor(self, obs=None, noise=None, reuse=False, scope="pi"):
		...
		with tf.variable_scope(scope, reuse=reuse):
			if self.feature_extraction == "cnn":
				...
				input_ = tf.concat([pi_h,noise],axis=-1)
		...
		self.logp_pi_new = logp_pi_new

		return deterministic_policy, policy, logp_pi_new,  mu_, log_std, pi_ # last four elements not useful
	...
	def proba_step(self, obs, noise, state=None, mask=None):
		return self.sess.run([self.policy, self.logp_pi_new], {self.obs_ph: obs, self.noise_ph: noise})

	def proba_noise(self, obs, state=None, mask=None):
		return self.sess.run([self.noise, self.logp_pi_noise], {self.obs_ph: obs})
