...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent_infer(self, infer_latent_vector, init_scale=1.0, init_bias=0.0, std_normal=False, prior_std=0):
        if std_normal:
            pdparam = tf.concat([tf.zeros([1, self.size]), prior_std*tf.ones([1, self.size])], axis=1)
            mean = tf.zeros([1,self.size])
            return self.proba_distribution_from_flat(pdparam), mean
        else:  
            mean = linear(infer_latent_vector, 'infer', self.size, init_scale=init_scale, init_bias=init_bias)
            logstd = tf.get_variable(name='infer/logstd', shape=[1, self.size], initializer=tf.zeros_initializer())
            pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
...
class DiagGaussianProbabilityDistribution(ProbabilityDistribution):
    ...
    def sample(self): # different from distributions.py
        ...
        stdn_gen = tf.random_normal(tf.shape(self.mean), dtype=self.mean.dtype)
        return self.mean + self.std * stdn_gen, stdn_gen
