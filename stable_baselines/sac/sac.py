...
class SAC(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, env2, callback=None, seed=None, path=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if self.verbose >= 1 and done and log_interval is not None and len(episode_rewards) % log_interval == 0:
                    ...
                    rew_evaluate = self.evaluate(env2)
                    with open(path,'a') as f1:
                        f1.write("%f " % rew_evaluate)                    
    ...
    def evaluate(self, env2):
        obs = env2.reset()
        rew = 0
        while True:
            action = self.policy_tf.step(obs[None], deterministic=True).flatten()
            rescaled_action = action * np.abs(self.action_space.low)
            obs, reward, done, _ = env2.step(rescaled_action)
            rew += reward
            if done:
                break
        return rew
